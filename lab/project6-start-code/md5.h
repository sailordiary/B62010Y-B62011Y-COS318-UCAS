...
char* digestFileMD5(char const *filename )
{
  MD5Init() ;
  ...
  if ( (file = fopen (filename, "rb")) == NULL ) {
    ...
    // ALOGD("%s can't be opened\n", filename );
    // throw_code_exception(env, "cannot open file backupDB");
    return NULL;
  }
  else
  {
  ...
  // ALOGD("MD5 : %s", digestChars );
  return digestChars ;
}

char* digestBuffer(FILE *file) {
  MD5Init();
  int len;
  unsigned char buffer[1024] ;
